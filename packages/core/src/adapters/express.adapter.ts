...
import type { ErrorContext, ResolvedErrorHandlerConfig } from "../exceptions/error-handler.provider";
import { DEFAULT_ERROR_HANDLER_CONFIG } from "../exceptions/error-handler.provider";
import type { AnyRouteConfig } from "../router/route-storage.types";
import type { ServerAdapter, StaticRouteResponse } from "./server-adapter.interface";
        ...
        if (route.middleware && route.middleware.length > 0) {
          for (const mw of route.middleware) {
            if (typeof mw !== "function") {
              console.warn(`[Nural] Skipping non-function middleware on ${route.method} ${route.path}:`, typeof mw);
              continue;
            }
