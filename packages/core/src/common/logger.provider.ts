...
import { getContextValue } from "./async-storage.provider";
...
abstract class Transport {
  ...
  protected formatJson(entry: LogEntry): string {
    ...
    let finalEntry: Record<string, unknown> = { ...entry, message: cleanMessage };
  ...
  protected formatPretty(entry: LogEntry): string {
    ...
    let suffix = "";
    const remainingData: unknown[] = [];

    if (data && Array.isArray(data)) {
      for (const item of data) {
        if (typeof item === 'object' && item !== null && 'userAgent' in item) {
          const record = item as Record<string, unknown>;
          suffix += ` - ${colors.gray}${String(record['userAgent'])}${colors.reset}`;
        } else {
