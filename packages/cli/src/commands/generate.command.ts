    ...
    if (schematic && !["middleware", "guard", "interceptor", "filter", "provider", "service", "resource"].includes(schematic)) {
      ...
      if (!schematic) {
        schematic = await CliPrompts.select(
          ...
          ['resource', 'middleware', 'guard', 'interceptor', 'filter', 'provider']
        );
      }
      if (!name) {
  ...
  try {
    ...
    if (schematic === "resource") {
      ...
      const files = [
        ...
        { tpl: "dto/create-name.dto.ts.ejs", out: `dto/create-${fileName}.dto.ts` },
        { tpl: "dto/update-name.dto.ts.ejs", out: `dto/update-${fileName}.dto.ts` },
        { tpl: "dto/index.ts.ejs", out: `dto/index.ts` },
        { tpl: "schema.response.ts.ejs", out: `schemas/${fileName}.response.ts` },
        { tpl: "schemas/index.ts.ejs", out: `schemas/index.ts` },
        { tpl: "models/index.ts.ejs", out: `models/index.ts` },
    ...
    if (fs.existsSync(destPath)) {
      CliLogger.error(`File already exists at ${destPath}`);
      return;
    }
